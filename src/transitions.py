...
class Configuration:
    ...
    def __str__(self):
        ...
        stackStr = printStack(self.stack, True)
        if self.buffer:
            idx = 0
            buffStr = '[ B0: ' + self.buffer[0].text
            if len(self.buffer) > 1:
                buffStr += ', ' + 'B1: ' + self.buffer[1].text
...
def printStack(elemlist, root=False):
    result = '['
    idx = len(elemlist) - 1
    for elem in elemlist:
        if isinstance(elem, Token):
            if root:
                result += ' S' + str(idx) + ': ' + elem.text + ', '
            else:
                result += elem.text + ', '
        elif isinstance(elem, list):
            if root:
                result += ' S' + str(idx) + ': ' + printStack(elem)
            else:
